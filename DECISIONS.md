...
6. **One thing we knowingly ship wrong**: Multi-intent emails (e.g., "evaluate your platform AND co-host a webinar") are always sent to triage as a single task instead of being split into two tasks routed to different owners. We chose this because splitting requires understanding task boundaries reliably, which the LLM does inconsistently. The safer default is one triage task that a human can split manually. The cost is that Farhan-style emails take longer to reach the right people.
